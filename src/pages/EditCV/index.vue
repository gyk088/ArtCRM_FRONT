  ...
  <div class="edit-cv-page">
    <div class="page-header">
      ...
      <div v-if="!isNew && cvRecord" class="header-meta">
        <span class="meta-chip">Обновлено {{ formatDate(cvRecord.updatedAt) }}</span>
      </div>
    </div>
    ...
    <div class="edit-container">
        ...
        <section class="form-section">
          ...
          <div class="form-item">
            <div class="field-label-row">
              <label class="field-label">Текст резюме</label>
      ...
      <div class="right-column">
        ...
        <div class="preview-card">
          ...
          <p v-else class="preview-text preview-placeholder">Текст резюме появится здесь по мере ввода...</p>
        </div>
...
const cvRecord = computed(() => (isNew.value ? null : cvStore.getCvById(route.params.id)))

const form = reactive({ title: '', text: '' })

if (!isNew.value && cvRecord.value) {
  form.title = cvRecord.value.title
  form.text = toEditableHtml(cvRecord.value.text)
}
...
const editor = useEditor({
  ...
  extensions: [
    StarterKit,
    Underline,
    Placeholder.configure({ placeholder: 'Опыт, образование, выставки, достижения...' }),
  ],
...
function save() {
  if (!form.title.trim() || !plainLength.value) {
    message.warning('Пожалуйста, заполните название и текст резюме.')
    return
  }
  if (isNew.value) {
    cvStore.addCv({ title: form.title.trim(), text: form.text })
    message.success('Резюме добавлено')
  } else {
    cvStore.updateCv(route.params.id, { title: form.title.trim(), text: form.text })
    message.success('Резюме обновлено')
  }
  router.push('/home/cv')
}

function remove() {
  cvStore.removeCv(route.params.id)
...
async function handleDownloadPdf() {
  ...
  try {
    await downloadCvPdf({ title: form.title, text: form.text })
  } finally {
